...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *heightConstraintOfPayButton;
...
- (void)viewDidLoad {
    ...
    [self.goPayButton setBackgroundImage:[UIImage imageWithColor:NAVIGATIONBAR_COLOR] forState:UIControlStateNormal];
    [self.goPayButton setBackgroundImage:[UIImage imageWithColor:[UIColor whiteColor]] forState:UIControlStateFocused];
    [self.goPayButton setBackgroundImage:[UIImage imageWithColor:[UIColor whiteColor]] forState:UIControlStateHighlighted];
    [self.goPayButton setBackgroundImage:[UIImage imageWithColor:[UIColor whiteColor]] forState:UIControlStateSelected];
    ...
    [XJOrderModel orderDetail:self.orderId handler:^(id object, NSString *msg) {
        if (object) {
            ...
            GJCFAsyncMainQueue(^{
                ...
                if (self.orderModel.orStatus.integerValue == 1) {
                    self.heightConstraintOfPayButton.constant = 45.f;
                } else {
                    self.heightConstraintOfPayButton.constant = 0;
                }
            });
        } else {
