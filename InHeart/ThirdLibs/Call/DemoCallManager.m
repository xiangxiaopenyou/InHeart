...
- (void)callDidReceive:(EMCallSession *)aSession
{
    ...
    @synchronized (_callLock) {
        ...
        self.currentController.timeBlock = ^(NSInteger time) {
            DemoCallManager *strongSelf = weakSelf;
            strongSelf.callTime = time;
        };
             ...
             error:(EMError *)aError
{
    ...
    if (aReason == EMCallEndReasonHangup) {
        if (self.callTime > 0) {
            NSDictionary *tempDictionary = @{@"callTime" : @(self.callTime), @"username" : aSession.remoteName};
            [[NSNotificationCenter defaultCenter] postNotificationName:@"videoCallDidEnded" object:tempDictionary];
        }
    ...
    void (^completionBlock)(EMCallSession *, EMError *) = ^(EMCallSession *aCallSession, EMError *aError){
        ...
        if (strongSelf) {
            ...
            @synchronized (self.callLock) {
                ...
                strongSelf.currentController.timeBlock = ^(NSInteger time) {
                    weakSelf.callTime = time;
                };
                dispatch_async(dispatch_get_main_queue(), ^{
